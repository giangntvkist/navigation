...
void quintic_bezier_splines(vector<Point>& path) {
    ...
    Vector tangent[num_waypoints];
    Vector second_der[num_waypoints];
    Vector n1, n2, n;
    double r1, r2;
    ...
    for(int i = 0; i < num_waypoints-1; i++) {
        double d = sqrt(pow(path[i].x - path[i+1].x, 2) + pow(path[i].y - path[i+1].y, 2));
        if (d > 1e-4) {
    ...
    tangent[0].nx = k_curve*(path[1].x - path[0].x); 
    tangent[0].ny = k_curve*(path[1].y - path[0].y);

    tangent[num_waypoints-1].nx = k_curve*(path[num_waypoints-1].x - path[num_waypoints-2].x);
    tangent[num_waypoints-1].ny = k_curve*(path[num_waypoints-1].y - path[num_waypoints-2].y);

    for(int i = 1; i < num_waypoints-1; i++){
        n1.nx = (path[i-1].x - path[i].x)/dis_point[i-1];
        n1.ny = (path[i-1].y - path[i].y)/dis_point[i-1];
 
        n2.nx = (path[i+1].x - path[i].x)/dis_point[i];
        n2.ny = (path[i+1].y - path[i].y)/dis_point[i];
        ...
        tangent[i].nx = k_curve*dis_min*n.nx/vec_magnitude;
        tangent[i].ny = k_curve*dis_min*n.ny/vec_magnitude;
    }
    second_der[0].nx = -6*path[0].x - 4*tangent[0].nx - 2*tangent[1].nx + 6*path[1].x;
    second_der[0].ny = -6*path[0].y - 4*tangent[0].ny - 2*tangent[1].ny + 6*path[1].y;
    second_der[num_waypoints-1].nx = 6*path[num_waypoints-2].x + 2*tangent[num_waypoints-2].nx + 4*tangent[num_waypoints-1].nx - 6*path[num_waypoints-1].x;
    second_der[num_waypoints-1].ny = 6*path[num_waypoints-2].y + 2*tangent[num_waypoints-2].ny + 4*tangent[num_waypoints-1].ny - 6*path[num_waypoints-1].y;
    for(int i = 1; i < num_waypoints-1; i++) {
        anpha = dis_point[i]/(dis_point[i] + dis_point[i-1]);
        beta = dis_point[i-1]/(dis_point[i] + dis_point[i-1]);
        second_der[i].nx = anpha*(6*path[i-1].x + 2*tangent[i-1].nx + 4*tangent[i].nx - 6*path[i].x) + beta*(-6*path[i].x - 4*tangent[i].nx - 2*tangent[i+1].nx + 6*path[i+1].x);
        second_der[i].ny = anpha*(6*path[i-1].y + 2*tangent[i-1].ny + 4*tangent[i].ny - 6*path[i].y) + beta*(-6*path[i].y - 4*tangent[i].ny - 2*tangent[i+1].ny + 6*path[i+1].y);
    }
    for(int i = 0; i < num_waypoints-1; i++){
        P0.x = path[i].x;
        P0.y = path[i].y;
...
void numerical_velocity_profile(double V_init = 0, double V_end = 0) {
    double L_h, L;
    double del_s, del_length;
    double length[num_waypoints-1];
    int k;
    double u, eps;
    double c;
    vector<double> v;
    ...
    U.clear();
    arc_length.clear();
    quintic_bezier_splines(global_path);
    L = 0;
    for(int i = 0; i < num_waypoints-1; i++) {
...
State bezier_trajectory(double t) {
    ...
    for(int i = 0; i < M; i++) {
        if(t >= T[i] && t < T[i+1]) {
            ...
            q_t.x = c0[idx].x + c1[idx].x*u + c2[idx].x*pow(u, 2) + c3[idx].x*pow(u, 3) + c4[idx].x*pow(u, 4) + c5[idx].x*pow(u, 5);
            q_t.y = c0[idx].y + c1[idx].y*u + c2[idx].y*pow(u, 2) + c3[idx].y*pow(u, 3) + c4[idx].y*pow(u, 4) + c5[idx].y*pow(u, 5);
            q_t.theta = lz_pose.theta;
            
            q_t.vx = (c1[idx].x + 2*c2[idx].x*u + 3*c3[idx].x*pow(u, 2) + 4*c4[idx].x*pow(u, 3) + 5*c5[idx].x*pow(u, 4))*u_dot;
    	    q_t.vy = (c1[idx].y + 2*c2[idx].y*u + 3*c3[idx].y*pow(u, 2) + 4*c4[idx].y*pow(u, 3) + 5*c5[idx].y*pow(u, 4))*u_dot;
            q_t.w = 0;
        } else if(t > T[M]) {
            idx = num_waypoints - 2;
            u = 1;
...
State scurve_trajectory(double t) {
    ...
    State q_t;
    for(int i = 0; i < num_waypoints-1; i++) {
        kx = global_path[i+1].x - global_path[i].x;
        ky = global_path[i+1].y - global_path[i].y;
        ...
        q_t.x = global_path[i].x + kx * s; q_t.y = global_path[i].y + ky * s;
    }
    return q_t;
}