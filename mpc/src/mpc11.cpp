...
int main(int argc,char **argv) {
    ...
    SX obj = SX::mtimes(SX::mtimes((q_N - P(Slice(0, 4),N_predictsize)).T(), Q_N), (q_N - P(Slice(),N_predictsize)));
    SX cst_states = X(Slice(),0) - P(Slice(),0);
    ...
    DM x0 = DM::repmat(DM::zeros(4, 1),1,N_predictsize+1);
    SX obj, cst;
    ...
    while(ros::ok()) {
        ...
        if(trajectory_ && lzpose_ && control_ && obstacles_) {
            ...
            p_(Slice(),0) = {lz_pose.x, lz_pose.y, v_0x, v_0y, lz_pose.theta};
