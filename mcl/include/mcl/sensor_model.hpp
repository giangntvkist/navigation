...
void scan_handle(pf_set_sample_t& s, sensor_msgs::LaserScan& scan_t, nav_msgs::OccupancyGrid& map) {
    ...
    if(converged) {
        int count_scan = 0;
        set_scan_t.ranges.clear();
        for(int i = 0; i < scan_t.ranges.size(); i++) {
            int count_particles = 0;
            for(int j = 0; j < s.samples.size(); j++) {
                pose_lidar.v[0] = s.samples[j].pose.v[0] + laser_pose_x*cos(s.samples[j].pose.v[2]) - laser_pose_y*sin(s.samples[j].pose.v[2]);
                pose_lidar.v[1] = s.samples[j].pose.v[1] + laser_pose_x*sin(s.samples[j].pose.v[2]) + laser_pose_y*cos(s.samples[j].pose.v[2]);
                beam_angle = inv_scan*(i*throttle_scan * angle_increment - angle_max) + s.samples[j].pose.v[2] + laser_pose_theta;

                pose_endpoint.v[0] = pose_lidar.v[0] + scan_t.ranges[i]*cos(beam_angle);
                pose_endpoint.v[1] = pose_lidar.v[1] + scan_t.ranges[i]*sin(beam_angle);
                ...
                if(map_valid(idx, idy) && set_occ_dist[id_cell] > beam_skip_distance) {
                    count_particles += 1;
                }else if(!map_valid(idx, idy)){
                    count_particles += 1;
                }
            }
            if(count_particles > beam_skip_threshold*s.samples.size()) {
                ...
                range.v[0] = scan_t.ranges[i];
                range.v[1] = i*throttle_scan*angle_increment - angle_max;
                set_scan_t.ranges.push_back(range);
            }
        }
        if(count_scan > beam_skip_error_threshold*scan_t.ranges.size()) {
            set_scan_t.ranges.clear();
            for(int i = 0; i < scan_t.ranges.size(); i++) {
                range.v[0] = scan_t.ranges[i];
                range.v[1] = i*throttle_scan*angle_increment - angle_max;
...
double ray_tracing(double x_, double y_, double anpha, nav_msgs::OccupancyGrid& map) {
    ...
    if(steep) {
        if(!map_valid(id_y, id_x) || map.data[id_x*map.info.width + id_y] != free)
            return map.info.resolution*sqrt(pow(id_y - id_y0, 2) + pow(id_x - id_x0, 2));
    }else {
        if(!map_valid(id_x, id_y) || map.data[id_y*map.info.width + id_x] != free)
...
double likelihoodfield_model(pf_scan_t& scan_t, pf_vector_t& q_t, nav_msgs::OccupancyGrid& map) {
    ...
    pf_vector_t pose_lidar, pose_endpoint;
    double beam_angle;
    pose_lidar.v[0] = q_t.v[0] + laser_pose_x*cos(q_t.v[2]) - laser_pose_y*sin(q_t.v[2]);
    pose_lidar.v[1] = q_t.v[1] + laser_pose_x*sin(q_t.v[2]) + laser_pose_y*cos(q_t.v[2]);
    for(int i = 0; i < scan_t.ranges.size(); i ++) {
        if(scan_t.ranges[i].v[0] < range_max && scan_t.ranges[i].v[0] >= range_min) {
            beam_angle = inv_scan*scan_t.ranges[i].v[1] + q_t.v[2] + laser_pose_theta;
            pose_endpoint.v[0] = pose_lidar.v[0] + scan_t.ranges[i].v[0]*cos(beam_angle);
            pose_endpoint.v[1] = pose_lidar.v[1] + scan_t.ranges[i].v[0]*sin(beam_angle);
            ...
            q *= a*exp(-0.5*pow(d_min/sigma_hit, 2));
        }
    }
    return q;
}
...
double beams_model(pf_scan_t& scan_t, pf_vector_t& q_t, nav_msgs::OccupancyGrid& map) {
    ...
    double x0, y0;
    x0= q_t.v[0] + laser_pose_x*cos(q_t.v[2]) - laser_pose_y*sin(q_t.v[2]);
    y0 = q_t.v[1] + laser_pose_x*sin(q_t.v[2]) + laser_pose_y*cos(q_t.v[2]);
    for(int i = 0; i < scan_t.ranges.size(); i++) {
        beam_angle = inv_scan*scan_t.ranges[i].v[1] + q_t.v[2] + laser_pose_theta;
        range_ = ray_tracing(x0, y0, beam_angle, map);

        c = 1/sqrt(2*sigma_hit*sigma_hit);
        upper_bound = range_max - range_;
        lower_bound = 0 - range_;

        eta_hit = 2/(erf(upper_bound*c) - erf(lower_bound*c));
        if(scan_t.ranges[i].v[0] >= 0 && scan_t.ranges[i].v[0] <= range_max) {
            p_hit = eta_hit*c/(sqrt(M_PI))*exp(-pow(c*(scan_t.ranges[i].v[0] - range_), 2));
            p_rand = 1/range_max;
        }else {
            p_hit = 0;
            p_rand = 0;
        ...
        eta_short = 1/(1 - exp(-lamda_short*range_));
        if(scan_t.ranges[i].v[0] >= 0 && scan_t.ranges[i].v[0] <= range_) {
            p_short = eta_short*lamda_short*exp(-lamda_short*scan_t.ranges[i].v[0]);
        }else {
            p_short = 0;
        }
        if(scan_t.ranges[i].v[0] == range_max) {
            p_max = 1;
        }else {
            p_max = 0;
        }
